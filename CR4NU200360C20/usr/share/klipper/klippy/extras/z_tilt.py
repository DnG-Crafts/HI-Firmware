...
class ZAdjustHelper:
    ...
    def adjust_steppers(self, adjustments, speed):
        ...
        for i in range(len(positions)-1):
            ...
            if z_low + next_stepper_offset >= curpos[2]:
                # 目标z位置大于当前z位置,表示热床要下降
                if stepper.get_name() == "stepper_z":
                    z_tilt.stepper_z_adjustment += abs(adjustments[1]-adjustments[0])
                elif stepper.get_name() == "stepper_z1":
                    z_tilt.stepper_z1_adjustment += abs(adjustments[1]-adjustments[0])
            else:
                if stepper.get_name() == "stepper_z":
                    z_tilt.stepper_z_adjustment += -abs(adjustments[1]-adjustments[0])
                elif stepper.get_name() == "stepper_z1":
            ...
            difference_z_z1 = abs(z_tilt.stepper_z_adjustment - z_tilt.stepper_z1_adjustment)
            if difference_z_z1 > 5:
                err_msg = """{"code":"key357", "msg":"The difference between z and z1 exceeds the threshold", "values":[]}"""
...
class ZTilt:
    def __init__(self, config):
        ...
        self.stepper_adjustment_path = "/mnt/UDISK/creality/userdata/config/stepper_adjustment.json"
        self.real_zmax_path = "/mnt/UDISK/creality/userdata/config/real_zmax.json"
    ...
    def cmd_ADJUST_STEPPERS(self, gcmd):
        ...
        logging.info(f"XXXXXXXXXXXXstepper_adjustment_path:{self.stepper_adjustment_path} adjustments:{adjustments}")
        if adjustments:
