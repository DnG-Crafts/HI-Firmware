...
CML_ERR_CODE_Z1_Z_AXIS_LINE_ERROR    = {'code':'key409', 'msg':'CHECK_MOTOR_LINE: Z-Z1-axis motor line is not inserted', 'values':[]}

class TMC_LINE_CHECK:
    ...
    def cmd_read_reg(self, gcmd):
        ...
        stepper_x_obj = self.printer.lookup_object('tmc2208 stepper_%s' % stepper)
        val = stepper_x_obj.mcu_tmc.test_get_register(addr)
        self.gcode.respond_info('tmc2208 stepper_%s reg val:'%stepper + hex(val))
        return val
        pass
    ...
    def cmd_check_z_motor_line(self, gcmd):
        ...
        self.delay_s(0.5)
        stepper_z_obj = self.printer.lookup_object('tmc2208 stepper_z')
        val = stepper_z_obj.mcu_tmc.test_get_register(111)
        self.gcode.respond_info('tmc2208 stepper_z reg val:'+ hex(val))
        flag_err = 0
        if((val & 0xff) != 0):
            self.delay_s(0.5)
            flag_err = 1
        ...
        #
        self.delay_s(0.5) 
        stepper_z1_obj = self.printer.lookup_object('tmc2208 stepper_z1')
        val = stepper_z1_obj.mcu_tmc.test_get_register(111)
        self.gcode.respond_info('tmc2208 stepper_z1 reg val:'+ hex(val))
        if((val & 0xff) != 0):
            self.delay_s(0.5)
            self.gcode.run_script_from_command('FORCE_MOVE STEPPER=stepper_z DISTANCE=-0.5 VELOCITY=10')
            flag_err |= 2
        ...
        if flag_err == 0:
            ...
            self.ck_and_raise_error(CML_ERR_CODE_Z1_Z_AXIS_LINE_ERROR)
        self.gcode.respond_info('Z-axis Z1-axis motor line is inserted ok!!!!!')

def load_config(config):
    return TMC_LINE_CHECK(config)